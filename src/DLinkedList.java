public class DLinkedList {
	...
	// Part 4: complete
	/**
	 * Swaps head and tail node values, and then updates the pointers so nextNode and 
	 * prevNode are swapped for all nodes.
	 * This action reverses the order of the doubly linked list.
	 */
	public void reverse(){
		
		// The following code reverses the value of the headNode and tailNode
		Node changeValues = headNode;
		headNode = tailNode;
		tailNode = changeValues;
		
		// The following code swaps nextNode and prevNode values for all Nodes.
		Node pointer = headNode;

		while(pointer!=null){
			changeValues = pointer.nextNode;
			pointer.nextNode = pointer.prevNode;
			pointer.prevNode = changeValues;
